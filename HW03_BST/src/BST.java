...
public class BST {
	...
	public boolean hasGreaterValueThan(int v) {
		TreeIterator temp = this.findMax(root);
		if(temp != null && temp.currentNode.data > v){
			return true;
		}
		return false;
	}
	...
	public BST difference(BST t) throws Exception {
		...
		while(temp.size() != 0){
			...
			TreeIterator itr = (TreeIterator) this.find(d);
			if(itr.currentNode.right != null){
				temp.add(itr.currentNode.right.data);
			}
			if(itr.currentNode.left != null){
				temp.add(itr.currentNode.left.data);
			}
		}
		BST s = new BST();
