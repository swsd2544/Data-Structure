...
public class BST {
	...
	public void cloneTree(BST tt) {
		...
		while(data.size() != 0){
			int d = data.remove(0);
			TreeIterator itr = (TreeIterator) tt.find(d);
			if(itr.currentNode.left != null) {
				data.add(itr.currentNode.left.data);
				this.insert(itr.currentNode.left.data);
			}
			if(itr.currentNode.right != null){
				data.add(itr.currentNode.right.data);
				this.insert(itr.currentNode.right.data);
			}
		}
	}
	...
	public int findNextData(int n) throws Exception {
		BSTNode temp = root;
		if(temp == null || findMax(temp).currentNode.data <= n) {
			return n;
		}
		int ans = findMax(temp).currentNode.data;
		while(ans != n + 1){											// If ans is nearest to n while greater than n, return ans
			if(temp.data > n){											// If temp is greater than n and min of leftsidetree is still greater than n, temp = temp.left -->
				ans = temp.data;															// Since member of the leftsidetree always less than temp,
				if(temp.left != null && findMax(temp.left).currentNode.data > n){			// if findMax(leftsidetree) is greater than n, there is a node that more nearer to n than temp
					temp = temp.left;
				} else {
					break;
				}
			} else if(temp.right != null){		//Check only root whether it is less than n
				temp = temp.right;
			} else {		//No more node to check
				break;
			}
		}
		return ans;
